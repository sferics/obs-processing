...
def parse_all_BUFRs_flat( source=None, file=None, known_stations=None, pid_file=None ):
    #TODO
    """
    Parameter:
    ----------
    ...
    for FILE in files_to_parse:
        ...
        with open(bufr_dir + FILE, "rb") as f:
            ...
            #iterid = ec.codes_keys_iterator_new(bufr, namespace=None)
            #if debug: print(ec.codes_get_message(bufr))
            
            #TODO does this have an influence??? ec.codes_skip_duplicates
            for fun in (ec.codes_skip_computed, ec.codes_skip_function): fun(iterid)
            ...
            while ec.codes_bufr_keys_iterator_next(iterid):
                ...
                if debug:
                    ...
                    obs_debug.append((key,value)); continue
                ...
                if valid_obs:
                    ...
                    if clear_key in bufr_mod_keys:
                        ...
                        if clear_key == "delayedDescriptorReplicationFactor":
                            if value == 10:
                                skip_next = 10
                                if debug: print(obs[ID][location])
...
def parse_all_BUFRs( source=None, file=None, known_stations=None, pid_file=None ):
    ...
    for FILE in files_to_parse:
        ...
        with open(bufr_dir + FILE, "rb") as f:
            ...
            while ec.codes_bufr_keys_iterator_next(iterid):
                ...
                if valid_obs:
                    ...
                    if clear_key in bufr_mod_keys:# and not ec.codes_is_missing(bufr, key):
                        ...
                        if value in null_vals: continue
                        # skip 1min ww and RR which are reported 10 times; 10min resolution is sufficient for us
                        if clear_key == "delayedDescriptorReplicationFactor":
                            if value == 10:
                                skip_next = 10
                                if debug: print(obs[ID][location][datetime])
                                try:    del obs[ID][location][datetime][-1]
                                except: pass
                            continue

                        #obs[ID][location][datetime].append( (clear_key, value) )
                        try:    obs[ID][location][datetime].append( (clear_key, value) )
                        except: obs[ID][location][datetime] = []

                        # avoid duplicate modifier keys (like timePeriod or depthBelowLandSurface) 
                        if clear_key in modifier_keys and value != -10:
                            try:
                    ...
                    if not subset and key in typical_keys:
                    ...
                    elif clear_key in station_keys:
                        meta[clear_key] = ec.codes_get(bufr, key)
                        #try:    meta[clear_key] = ec.codes_get(bufr, key)
                        #except: meta[clear_key] = ec.codes_get_array(bufr, key)[0]
                        #TODO some OGIMET-BUFRs seem to contain multiple station numbers in one key (arrays)
                        ...
                        if meta[clear_key] in meta_null_vals:
                            del meta[clear_key]; continue
                        ...
                        if location:
                            ...
                            else: prev_location = copy(location); obs[ID][location] = {}
                        elif subset > 0 and prev_location is not None: location = copy(prev_location)

                    elif location:

                        if clear_key in time_keys:
                            meta[clear_key] = ec.codes_get_long(bufr, key)
                            if meta[clear_key] in null_vals: del meta[clear_key]
        ...
        if memory_free <= config_script["min_ram"]:
            ...
            if obs_db:
                gf.obs_to_station_databases( obs_db, output_path, "raw", max_retries, timeout_station, verbose )
...
if __name__ == "__main__":
    ...
    #read yaml configuration file (default: config.yaml) into dictionary
    config          = gf.read_yaml( args.config )
    script_name     = os.path.basename(__file__)
    config_script   = config["scripts"][script_name]
    conda_env = os.environ['CONDA_DEFAULT_ENV']
   
    #TODO change this part so that command line arguments just overwrite config_script
    ...
    meta_null_vals = copy(null_vals)
    for i in config_script["null_vals"]: meta_null_vals.add(i)
    #meta_null_vals = set(config_script["null_vals"])
    ...
    # get the keys from the conversion dictionary, skip the first 6 which are used for unit conversion / as helpers
    bufr_translation_keys = list(bufr_translation)[6:]
    ...
    for i in bufr_translation_keys:
    ...
    #modifier_time_keys = frozenset( modifier_keys | {"timePeriod"} )
    ...
    bufr_mod_keys           = frozenset( bufr_keys | modifier_keys )
    relevant_keys           = frozenset( bufr_mod_keys | station_keys | typical_keys | time_keys )

    # get source specific configurations
    if args.source:
    ...
    if not args.file:
        ...
        for SOURCE in config_sources:
            ...
            bf = bufr(script_name[-5:-3], config_script, config_bufr)
            """
            if verbose: print(f"Parsing source {SOURCE}...")
            
            #bf.parse_all( source = SOURCE, pid_file=pid_file )
