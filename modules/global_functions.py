...
def read_yaml(file_name="obs", file_dir="config", ext="yml", typ="safe", pure=True,
    ...
    #TODO add numpy arrays, pandas/polars DataFrames and other useful datatypes?
...
def get_input_files_dict( config_database, input_files=[], source="extra", config_source: dict={},
        PID: int=0, redo=False, restart=False, log=None, verbose=False, debug=False ):
    """
    Parameter:
    ----------
    ...
    if input_files:
        ...
        for file_name in tuple(files_to_parse):
            ...
            if restart:
                ...
                ID = db.register_file(file_name, source_dir, source, status_locked, file_date,
                        verbose=verbose)
                if not ID:
                    if log: log.error(f"REGISTERING FILE '{file_path}' FAILED!")
                    continue
