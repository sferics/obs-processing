...
from bufr_functions import clear, to_datetime, convert_keys_units
...
def parse_all_BUFRs( source=None, file=None, known_stations=None, pid_file=None ):
    ...
    for FILE in files_to_parse:
                        ...
                        if value not in null_vals:
                            ...
                            if clear_key in unit_keys:
                                ...
                                if clear_key == "timePeriod":
                                    value = abs(value)
                                    if unit == "min":
                                        if value >= 60: value //= 60; unit = "h"
                                    if value == 0 or (unit == "h" and value == 0):
                                        value = 0; unit = "s"
                                    value = str(value) + unit
                                    obs_data = ( clear_key, value )

                                else: obs_data = ( clear_key, value, unit )

                            else: obs_data = ( clear_key, value )
        ...
        """
        count = False
        Y,M,D,h,m = (0 for _ in range(5))

        for i in keys:
            if  count and i == "year":
                Y+=1
            elif count and i == "month":
                M+=1
            elif count and i == "day":
                D+=1
            elif count and i == "hour":
                h+=1
            elif count and i == "minute":
                m+=1
            elif i == "subsetNumber":
                count = True
                if Y > 1: print("Y:", hour)
                if M > 1: print("M:", month)
                if D > 1: print("D:", day)
                if h > 1: print("h:", hour)
                if m > 1: print("m:", minute)
                if not Y==M==D==h==m: print(FILE); sys.exit()
                Y,M,D,h,m = (0 for _ in range(5))
        """
        ...
        if memory_free <= config_script["min_ram"]:
            ...
            obs_db = convert_keys_units(obs,source,modifier_keys,unit_keys,height_depth_keys,bufr_translation,bufr_flags)
            if obs_db:
...
if __name__ == "__main__":
    ...
    if "d" in args.skip:    config_script["skip_duplicate"] = True
    ...
    modifier_keys, unit_keys, depth_keys, height_keys = (set() for _ in range(4))

    for i in bufr_translation:
        if type(bufr_translation[i]) == list and type(bufr_translation[i][0]) == dict:
            unit_keys.add(i)
        elif type(bufr_translation[i]) == dict:
    ...
    # skip cloud base; timePeriod key also needs a unit
    unit_keys = frozenset( unit_keys | {"timePeriod"} )
    ...
    relevant_keys = bufr_obs_time_keys | station_keys | typical_keys | time_keys | {"subsetNumber"}
